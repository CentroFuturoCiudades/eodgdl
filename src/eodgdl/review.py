...
COLUMNS = (KEYS[:2] + PERSON_COLUMNS + KEYS[2:]
           + [c for name in FIELDS for c in (name, new_column(name))]
           + READ_ONLY + [STATUS, new_column(STATUS), NOTE])
...
def chain_rows(cleaned: EODTables, shipped: EODTables) -> pd.DataFrame:
    ...
    out[ISSUE_FLAG] = Cr[ISSUE_FLAG].astype(object).fillna("") if ISSUE_FLAG in Cr else ""
...
def pending_persons(rows: pd.DataFrame, codes=None) -> pd.MultiIndex:
    ...
    flagged = pd.concat([has_code(rows[ISSUE_FLAG], c) for c in codes], axis=1).any(axis=1)
    return flagged.groupby(level=PERSON).any().loc[lambda s: s].index


def _hhmm(minutes: pd.Series) -> pd.Series:
    return minutes.map(lambda m: MISSING if pd.isna(m) else f"{int(m) // 60:02d}:{int(m) % 60:02d}")
...
def _person_info(hab: pd.DataFrame | None, persons: pd.MultiIndex) -> pd.DataFrame:
    ...
    src = {"sex": "sexo_nacimiento", "age": "edad", "occupation": "ocupacion", "repeated": "diario_repetido"}
...
def chain_sheet(rows: pd.DataFrame, hab: pd.DataFrame | None = None, persons=None) -> pd.DataFrame:
    ...
    if persons is not None:
        keep = pd.MultiIndex.from_arrays(
            [t.index.get_level_values(PERSON[0]), t.index.get_level_values(PERSON[1])]).isin(list(persons))
        t = t[keep]
    ...
    sheet[STATUS] = np.where(t.dropped, DROPPED, np.where(t[FIX_FLAG] != "", "changed", ""))
...
def sheet_edits(sheet: pd.DataFrame) -> pd.DataFrame:
    ...
    unknown = [c for c in s.columns if c.startswith(NEW) and c[len(NEW):] not in EDITABLE]
    if unknown:
        raise ValueError(f"not a column the sheet reads back: {unknown}; the new columns are "
                         f"{[new_column(c) for c in EDITABLE]}")
    notes = s[NOTE].astype(str).str.strip() if NOTE in s else pd.Series("", index=s.index)
    ...
    for col in EDITABLE:
        ...
        if col == STATUS:
            ...
            diff = (((wanted == DROPPED) & (before != DROPPED)) | ((wanted == RESTORED) & (before == DROPPED))
...
def apply_edits(cleaned: EODTables, edits: pd.DataFrame, shipped: EODTables | None = None) -> EODTables:
    ...
    levels = {f.column: set(trips[f.column].cat.categories) if hasattr(trips[f.column], "cat") else None
              for f in FIELDS.values() if not isinstance(f.column, tuple)}
        ...
        if r.field == STATUS:
            ...
            if status == DROPPED:
                ...
                if key in trips.index or key in shipped_index:
                    ...
                    problems.append(f"{where}: restoring a dropped row needs the shipped tables")
                else:
                    problems.append(f"{where}: no such trip to restore")
            else:
                problems.append(f"{where}: new status must be '{DROPPED}' or '{RESTORED}', not '{value}'")
            continue
    ...
    if restored:
        ...
        if legs is not None:
            legs = pd.concat([legs, shipped.legs[shipped.legs.index.droplevel("folio_traslado").isin(restored)]])
    for keys, sign in ((dropped, -1), (restored, 1)):
        for hh, person in pd.MultiIndex.from_tuples([k[:2] for k in keys], names=PERSON).unique() if keys else []:
...
def _codes_of(flags: pd.Series) -> pd.Series:
    ...
    return flags.groupby(level=PERSON).agg(lambda s: ";".join(sorted({c for v in s for c in v.split(";") if c})))


def verify_edits(cleaned: EODTables, shipped: EODTables, edits: pd.DataFrame) -> Verification:
    ...
    persons = (
        ...
        .groupby(KEYS[:2])[["cells", "dropped", "restored", "notes"]].sum().astype(int)
    ...
    sel = lambda rows: rows[pd.MultiIndex.from_arrays(
        [rows.index.get_level_values(0), rows.index.get_level_values(1)]).isin(who)]
    ...
    made = [bool(set(a.split(";")) - set(b.split(";")) - {""}) for a, b in zip(persons.after, persons.before)]
    summary = {
    ...
    return Verification(revised, chain_sheet(after_rows, revised.hab, who), persons, summary)
